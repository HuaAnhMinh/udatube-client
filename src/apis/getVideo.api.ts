...
const getVideoApi = async (accessToken: string, videoId: string): Promise<SuccessResponse | ErrorResponse> => {
  try {
    const response = await api.get(endpoints.getVideo(videoId), {
      headers: {
